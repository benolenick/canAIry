...
_STARTUP_QUIET_SECONDS = 20.0
_MAX_ALERTS_PER_MINUTE = 25
...
class CanairyEventHandler(FileSystemEventHandler):
    ...
    def _in_startup_quiet_window(self) -> bool:
        return (time.monotonic() - self._started_monotonic) < _STARTUP_QUIET_SECONDS

    def _is_rate_limited(self) -> bool:
        now = time.monotonic()
        window_start = now - 60.0
        while self._alert_times and self._alert_times[0] < window_start:
            self._alert_times.popleft()
        if len(self._alert_times) >= _MAX_ALERTS_PER_MINUTE:
            return True
    ...
    def on_any_event(self, event: FileSystemEvent) -> None:
        ...
        # Ignore startup churn generated by canAIry itself while planting files.
        if self._in_startup_quiet_window():
            return

        # Ignore low-signal temp/lock/marker activity.
        if self._is_noise_path(src_path):
            return

        # Drop high-volume floods even if each path is unique.
